...
This is accomplished by the replacing the `power.t.test` function with the `power.ftest` function. As you can see below it operates very similiar to the `power.t.test`; the main differences are that `power.ftest` requires arguments for the degrees of freedom rather than sample size. Further, rather than allow for input of the power as an arugment, `power_ftest` allows the user to input the beta_level.
...
As we can see here we can tell that we are tad underpowered to detect the effect of interest. Now, let us use `power.ftest` to find the alpha that will get us our desired beta (in this case let us assume .2).
