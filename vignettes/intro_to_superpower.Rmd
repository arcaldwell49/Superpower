...
author: "Daniel Lakens & Aaron C. Caldwell"
date: "`r Sys.Date()`"
...
The goal of Superpower is to easily simulate single factor and factorial designs and empirically calculate power using a simulation approach. Currently, the pacakge can only calculate power for ANOVAs.
...
You can set the sample size **per condition** by setting a value for n. The assumption is that you will collect equal sample sizes in all conditions [expanding Supepower to allow different sample sizes in each group is a planned future option].
