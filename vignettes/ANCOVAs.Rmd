...
Additionally, user specified contrasts can also be used for a power analysis. These can be provided in the `contrasts` argument of the `ANCOVA_analytic` function or supplied directly to the `ANCOVA_contrast` function as an independent test. The `ANCOVA_analytic` function requires that the contrasts matricies are provided as matrices. The contrasts can be accessed in the `con_list` part of the results and can be named (in this case "test").
...
- TO BE ADDED. See @shieh_ancova for the exact mathematical explanation.
