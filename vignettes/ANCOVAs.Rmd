...
This function operates similar to the `ANOVA_power` and `ANOVA_exact` functions. However, the `ANCOVA_analytic` function doesn't require the use of `ANOVA_design` first. Please note that unlike the previous function there is no option to apply the approximation from @cohen_book for factorial designs
...
Additionally, user specified contrasts can also be used for a power analysis. These can be provided in the `contrasts` argument of the `ANCOVA_analytic` function or supplied directly to the `ANCOVA_contrast` function as an independent test.
