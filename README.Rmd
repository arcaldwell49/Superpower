...
* [Simulation-based power calculations](#simulation-based-power-calculations)
...
The goal of `superpower` is to easily simulate ANOVA designs and empirically calculate power using a simulation approach. 
This app is intended to be utilized for prospective (a priori) power analysis.

## Installation

You can install the released version of `superpower` from [GitHub](https://github.com/arcaldwell49/superpower) with:
...
Note that we did not specify the labelnames, and thus they are automatically created. 
...
The ANOVA_exact function takes the result from the ANOVA_design function, and simulates one dataset that exactly matches the desired properties (using the `mvrnorm` function from the `MASS` package, using the setting `empirical = TRUE`). This dataset is used to perform a single ANOVA, and the results are used to compute power (thanks to Chris Aberson for inspiring this approach).
...
Compared to the ANOVA_power function, the ANOVA_exact approach is much faster (it requires simulating only a single dataset). Currently the only difference is that ANOVA_exact does not allow you to examine the consequences of correcting for multiple comparisons (there is no `p_adjust` option), and that ANOVA_exact does not work for sample sizes smaller than 8 (because it is not possible to simulate a dataset with exactly the desired properties for very small sample sizes). 
...
The result for the power simulation is printed, and has two sections (which can be surpressed by setting verbose = FALSE). The first table provides power (from 0 to 100%) and effect sizes (partial eta-squared) for the ANOVA result. We see the results for the main effects of factor voice, emotion, and the voice*emotion interaction. 
...
Because the true pattern of means is always unknown, it is sensible to examine the power across a range of scenarios. For example, is the difference in means is somewhat smaller,because the means are 24 and 26 instead of 24 and 26.2, we can compare the power curve above with the power curve for a slightly less optimistic scenario (where the true effect size is slightly smaller).
...
It could be that in ddition to a slightly smaller effect size, the standard deviation is slighty larger than we expected as well. This will reduce the power even further, and is thus an even less optimistic scenario. Let's assume the true standard deviation is 6.8 instead of 6.4.
...
It is clear we would need to collect more participants in each group to increase our power. Let's plot a power curve across a range of sample sizes to see how much we need to increase the sample size. 

```{r, fig.width=7, fig.height=4}
plot_power(design_result, min_n = 10, max_n = 250)
```

We see 134 participants give us aroud 90% power for the ANOVA. But we should also check the power for the comparison of the control condition and the cat condition. 
...
knitr::include_graphics("vignettes/screenshots/gpower_9.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("vignettes/screenshots/gpower_1.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("vignettes/screenshots/gpower_11.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("vignettes/screenshots/gpower_10.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("vignettes/screenshots/gpower_12.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("vignettes/screenshots/gpower_14.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("vignettes/screenshots/gpower_5.png")
```
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("vignettes/screenshots/gpower_6.png")
```
...
# reduce file size for github - only save what is needed
# power_result_cross_80$sim_data <- NULL
# power_result_cross_80$plot1 <- NULL
# power_result_cross_80$plot2 <- NULL
# saveRDS(power_result_cross_80, file = "vignettes/sim_data/power_result_cross_80.rds")
power_result_cross_80 <- readRDS(file = "vignettes/sim_data/power_result_cross_80.rds")
...
# power_result_cross_40$sim_data <- NULL
# power_result_cross_40$plot1 <- NULL
# power_result_cross_40$plot2 <- NULL
# saveRDS(power_result_cross_40, file = "vignettes/sim_data/power_result_cross_40.rds")
power_result_cross_40 <- readRDS(file = "vignettes/sim_data/power_result_cross_40.rds")
...
# power_result_ordinal$sim_data <- NULL
# power_result_ordinal$plot1 <- NULL
# power_result_ordinal$plot2 <- NULL
# saveRDS(power_result_ordinal, file = "vignettes/sim_data/power_result_ordinal.rds")
power_result_ordinal <- readRDS(file = "vignettes/sim_data/power_result_ordinal.rds")
...
As we see, power is quite low with n = 20 per cell. Let's examine how the power increases, as the sample size per condition increases.
...
```{r, out.width = "600px", echo=FALSE}
knitr::include_graphics("vignettes/screenshots/PS2000.gif")
```
...
We see the 26.9 and 64.2, and 26.9 correspond to the results of the exact simulation. 
